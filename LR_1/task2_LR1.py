...
def rgb_to_grayscale_manual(image):
    ...
    start = time.time()
    digits = np.array([0.299, 0.587, 0.114], dtype=np.float32)

    #/0 - у высота /1- х ширина /2- цветовые каналы в порядке BGR
    # Суммируем вдоль axis=0 (складываем каналы) 
    gray = np.sum(digits * image, axis=2)
...
def convolve_manual(image, kernel):
    ...
    if image.ndim == 2:                                         # количество измерений (размерность) массива чбшка
        ...
        output = np.sum(windows * kernel, axis=(-1, -2))        #умножаем каждое  окно на ядро, для  каждого окна сложим все  числа в одно
...
def gaussian_kernel(size=5, sigma=1.0):
    """  Создаёт двумерное гауссово ядро для свёртки.
    
    Args:
        size: Размер ядра (нечётное число).
        sigma: Стандартное отклонение гауссова распределения.
    
    Returns:
        Нормализованное гауссово ядро заданного размера.
    """""
    ax = np.linspace(-size // 2, size // 2, size)               # одномерн сеткa координат
    xx, yy = np.meshgrid(ax, ax)                                # двумерн сеткa координат
                                                                # Создаёт две матрицы:
    ...
    return kernel / np.sum(kernel)                              # нормализуем,  Сумма всех элементов становится равна 1 Без нормализации изображение становится ярче/темнее          ┃
...
#  ВЫДЕЛЕНИЕ ГРАНИЦ (СОБЕЛЬ)  вычисление градиентов

def sobel_manual(image):
    """Ручная реализация оператора Собеля
    вычисляет изменения яркости между соседними пикселями, что позволяет определить, где находятся границы объектов.
    """
    sobel_x = np.array([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]], dtype=np.float32)  #(выявляет вертикальные границы)
...
def sobel_opencv(image):
    ...
    grad_y = cv2.Sobel(image, cv2.CV_32F, 0, 1, ksize=3) # cv2.CV_32F глубина выходного изображения (ddepth)
...
def main2():
    ...
    image_path = "paintings/435950_original.jpg"
